...
local LuauBytecode = require(script.Parent:WaitForChild("LuauBytecode"))
local IrisLoader = require(script.Parent:WaitForChild("IrisLoader"))
...
function IrisBytecodeViewer.start(config)
	...
	local Iris, irisError = IrisLoader.load(config, modulesFolder)
	if not Iris then
		error(irisError)
	end
